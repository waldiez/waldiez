...
describe("getNodePositionFromJSON", () => {
    it("should return the position value", () => {
        expect(getNodePositionFromJSON({ position: { x: 0, y: 0 } })).toEqual({
            x: 0,
            y: 0,
        });
        expect(getNodePositionFromJSON({ position: { x: 1, y: 1 } })).toEqual({
            x: 1,
            y: 1,
        });
        expect(getNodePositionFromJSON({ position: { x: 2, y: 2 } })).toEqual({
            x: 2,
            y: 2,
        });
    ...
    it("should return the position value with custom position", () => {
        ...
        expect(getNodePositionFromJSON({} as any, { x: 1, y: 1 })).toEqual({
            x: 1,
            y: 1,
        });
    });
    it("should check for position in .rest", () => {
        ...
        expect(
            getNodePositionFromJSON({
                rest: { position: { x: "1.5", y: "1.5" } },
            }),
        ).toEqual({
            x: 20,
            y: 20,
        });
        expect(getNodePositionFromJSON({ rest: {} as any })).toEqual({
            x: 20,
            y: 20,
        });
    });
