...
export const runCommandInDir = (
    ...
    let commandString = `${cmd} ${args.join(" ")}`.replace(__rootDir, ".");
    if (commandString.endsWith(" ")) {
        commandString = commandString.slice(0, -1);
    }
    let relativeToRootToLog = relativeToRoot;
    if (relativeToRoot === ".") {
        relativeToRootToLog = path.basename(__rootDir);
    }
...
const getRelativePathToRoot = (dir: string): string => {
    let relativeToRoot = path.relative(__rootDir, dir);
    if (relativeToRoot === "") {
        relativeToRoot = ".";
    }
